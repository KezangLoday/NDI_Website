...
directory. Paths below are written relative to the repository root — resolve
them against it rather than against whatever the working directory happens to
be. (Upstream ships this skill as a plugin and writes `${CLAUDE_PLUGIN_ROOT}/`
in front of every path; it is installed here as a project skill instead, where
